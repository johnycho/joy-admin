...
import com.nhn.pebblewebback.application.constant.EventType;
import com.nhn.pebblewebback.application.dto.SignatureDto.SignatureResponse;
...
public class SignatureService {
  ...
  private PrivateKey getPrivateKey(final String type) throws NoSuchAlgorithmException, InvalidKeySpecException {
    return switch (EventType.getTypeByCode(type)) {
      case COMMUNITY_NFT -> generatePrivateKey(pebbleWebProperties.getCommunityNft().getMessageSignerPrivateKey());
      case GIVEAWAY -> generatePrivateKey(pebbleWebProperties.getGiveaway().getMessageSignerPrivateKey());
    };
  }
